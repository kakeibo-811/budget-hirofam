...
export function renderAnalytics(root: HTMLElement) {
  ...
  async function load(month: string) {
    ...
    try {
      ...
      clear(content);
      content.appendChild(header(month, range, async (r) => {
        range = r;
        localStorage.setItem('analysisRange', String(r));
        await load(month);
      }));
      content.appendChild(el('div', { class: 'analytics-metrics' }, [
        metric(L('家計収支', 'Household balance'), (dash.income?.total || 0) - (dash.household?.total || 0), `${dash.period.start} - ${dash.period.end}`),
        ...
        metric(L('妻', 'Wife'), dash.settlement?.wife_salary_balance || 0, `${L('収入', 'Income')} ${formatYen(dash.income?.lisa || 0)} / ${L('残り', 'Remaining')} ${formatYen(dash.settlement?.wife_salary_balance || 0)}`),
      ]));
...
function header(month: string, range: number, setRange: (r: number) => void): HTMLElement {
  ...
  return el('section', { class: 'analytics-hero' }, [
    el('div', {}, [
      ...
      el('p', { class: 'muted' }, [L('25日から翌24日までの家計月で、収入・支出・口座不足を確認します。', 'Tracks income, spending, and cash shortfall by the 25-to-24 budget cycle.')]),
    ]),
    el('div', { class: 'range-card' }, [
      el('strong', {}, [L('表示期間', 'Range')]),
      el('div', { class: 'pill-row' }, buttons),
      el('div', { class: 'muted' }, [month]),
    ]),
  ]);
}

function metric(label: string, amount: number, sub: string): HTMLElement {
  const ok = amount >= 0;
  return el('div', { class: `metric ${ok ? 'metric-ok' : 'metric-warning'}` }, [
    el('div', { class: 'metric-label' }, [label]),
    el('div', { class: 'metric-value' }, [signedYen(amount)]),
...
function visualGraphCard(items: Dash[]): HTMLElement {
  const card = el('div', { class: 'card analytics-visual-card soft-card' }, [
    el('div', { class: 'section-head' }, [
      el('div', {}, [
        el('h3', {}, [L('収支グラフ', 'Visual cashflow graph')]),
        el('p', { class: 'muted' }, [L('夫婦別の収入と、家計全体の支出を月次で比較します。', 'Compares partner income and household spending by month.')]),
      ]),
    ]),
  ]);
...
function stackedSvg(items: Dash[]): HTMLElement {
  ...
  wrap.appendChild(legend([
    [L('夫収入', 'H income'), 'chart-income-h'],
    [L('夫支出', 'H outgo'), 'chart-out-h'],
    [L('妻収入', 'W income'), 'chart-income-w'],
    [L('家計支出', 'Household'), 'chart-out-w'],
  ]));
  return wrap;
}
...
function trendCard(items: Dash[]): HTMLElement {
  const card = el('div', { class: 'card' }, [el('h3', {}, [L('月次トレンド', 'Monthly trend')])]);
  ...
  for (const d of items) {
    bars.appendChild(el('div', { class: 'trend-row' }, [
      ...
      bar(L('夫支出', 'H out'), d.settlement?.husband_final_burden || 0, max),
      bar(L('妻収入', 'W inc'), d.income?.lisa || 0, max),
      bar(L('家計支出', 'Household'), d.household?.total || 0, max),
    ]));
  }
  card.appendChild(bars);
  const wrap = el('div', { class: 'table-wrap' });
  const table = el('table', { class: 'data compact-table' });
  table.innerHTML = `<thead><tr><th>${L('月', 'Month')}</th><th class="num">${L('夫収入', 'H income')}</th><th class="num">${L('夫支出', 'H outgo')}</th><th class="num">${L('妻収入', 'W income')}</th><th class="num">${L('家計支出', 'Household')}</th></tr></thead>`;
  const tb = el('tbody');
  for (const d of items) tb.appendChild(el('tr', {}, [
    ...
    el('td', { class: 'num' }, [formatYen(d.household?.total || 0)]),
  ]));
...
function judgementCard(items: Dash[]): HTMLElement {
  ...
  const table = el('table', { class: 'data compact-table' });
  table.innerHTML = `<thead><tr><th>${L('月', 'Month')}</th><th>${L('夫', 'Husband')}</th><th>${L('妻', 'Wife')}</th><th>${L('口座不足', 'Cash shortfall')}</th></tr></thead>`;
  const tb = el('tbody');
...
function timelineTable(rows: any[]): HTMLElement {
  ...
  for (const r of rows) tb.appendChild(el('tr', {}, [
    el('td', {}, [r.date]),
    el('td', {}, [r.label]),
    el('td', { class: 'num' }, [formatYen(r.amount)]),
    el('td', { class: 'num' }, [formatYen(r.balance_after)]),
  ]));
  table.appendChild(tb);
  wrap.appendChild(table);
  return wrap;
...
function topExpensesCard(dash: Dash): HTMLElement {
  ...
  for (const r of rows) tb.appendChild(el('tr', {}, [
    el('td', {}, [r.date]),
    el('td', {}, [r.description || '']),
    el('td', {}, [t(`payer.${r.burden_owner || r.payer}` as any)]),
    el('td', { class: 'num' }, [formatYen(r.amount)]),
  ]));
  table.appendChild(tb);
  wrap.appendChild(table);
